...
import { UilCheckCircle, UilEllipsisV, UilMinus } from '@iconscout/react-unicons'
import { useDispatch, useSelector,  } from 'react-redux';
import { GET_CHANNEL_DETAILS_OF_SELECTED_VIDEO, GET_SELECTED_VIDEO } from '../../redux/constants/constants';
import { useNavigate } from 'react-router-dom';
import { removeToFavorite } from '../../redux/actions/favoriteActions';



export default function VideoCardHorizontal(props) {
  const { id, snippet: { description, publishedAt, channelTitle, title, thumbnails, channelId } } = props?.videoData;
  const { viewFor } = props;
  ...
  const handleVideoClick = () => {
    navigate(`/watch/${id}`, { state: { channelData, videoData: moreDetailsOfVideo } });
  };

  const removeFormWatchLater = (e) => {
    e.stopPropagation();
  ...
  return (
    ...
    >
      <div className='col-md-4 position-relative m-0 p-0'
        style={{ width: viewFor === 'watch-later' && '10rem' }}
      >
        <LazyLoadImage
      ...
      <div className='col-md-8 pt-1 d-flex'>
      <div style={{
          width: viewFor === 'watch-later' ? '95%' : '100%'
      }}>
          <div className={`${viewFor === 'search' ? 'fs-3 py-1' : viewFor === 'watch-later' ? 'fs-6 mt-2' : 'fs-6 pt-n-1'} video-title`}>{title}</div>
          {
            viewFor === 'search' ?
              <>
              ...
              : viewFor === 'watch-later' ? <>
                <div className='d-flex justify-content-start py-0'>
                  <div className='d-flex pt-2 align-items-center' style={{ fontSize: '12px' }}>
        ...
        {
          viewFor === 'watch-later' ? <div 
            className='text-right'
            style={{width: '5%'}}>
            <div
              className='me-2 d-flex justify-content-center align-items-center rounded-circle'
              style={{ width: '2rem', height: '2rem', background: '#282828' }}
              onClick={removeFormWatchLater}
            ><UilMinus style={{ background: 'transparent' }} /></div>
          </div> : null
        }
      </div>
